...
const PrivateReportPage: React.FC = () => {
  ...
  const getIcon = (status: string) => {
    switch (status) {
      case "submitted":
        return <span className="pal-black">Status: Queued</span>;
      case "running":
        return <span className="pal-yellow">Status: In Progress</span>;
      case "failed":
        return <span className="pal-red">Status: Failed</span>;
      case "finished":
        return <span className="pal-green">Status: Success</span>;
      default:
        return null;
    }
  };
  ...
  useEffect(() => {
    const fetchSubmissionErrors = async () => {
      if (selectedSubmission === "development") {
        ...
        try {
          const errors = await SubmissionService.getSubmissionErrors(
          ...
          if (errors === "Invalid submission ID") {
            ...
            if (
              tempErrorData.error_type
                .toLowerCase()
                .includes("operation".toLowerCase()) ||
              tempErrorData.error_type
                .toLowerCase()
                .includes("wrapper".toLowerCase()) ||
              tempErrorData.error_type
                .toLowerCase()
    ...
    */
    const fetchSubmissionResults = async () => {
      ...
      if (typeof num !== "number") {
        ...
        SubmissionService.getSubmissionResults(num, token)
                ...
                >
                  ...
                  >
                    {submissionData.status === "finished"
                      ? `Success Rate: ${
                          100 - parseInt(errorData.error_rate || "0")
                        }%`
